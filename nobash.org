...
The configuration goes into some configuration file,
which is loaded at runtime from some place in the filesystem.

This has all the same problems as dynamically linking libraries.[fn:dynlibs]
At runtime, the configuration might not be there,
or it might be the wrong configuration,
or the wrong configuration version:
containing newer or older configuration parameters that don't match the software.

Instead, we can statically link our configuration.
...
rather than looking up in the filesystem.
...
we have one executable per environment we wish to run in.
...
With traditional configuration files, we would use complex templating to generate configuration files;
by writing our own main functions and executable,
we can store our shared configurations in libraries that we import and reuse,
through the normal code reuse patterns that are natural for our chosen language.
...
If we have, say, some software in C,
...
just serialize the configuration in some format[fn:serialization] and pass it directly over stdin
...
This single parent program can straightforwardly establish a TCP connection
and pass each half of that TCP connection down to different components it starts.[fn:fd_passing]
In this way, we can statically guarantee that each component is connected to the correct service.

In the long run,
more information must become statically visible
if we are to make large scale systems.
Extending "static linking" to configuration is one instance of this,
but there are many other ways in which we can make information statically visible,
and use it to decrease complexity and improve correctness of our systems.
